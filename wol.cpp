...
#ifdef DEBUG
            QMessageBox::warning(0, "WOL", QObject::tr("Invalid ethernet addres s!"));
#endif
            errno = EINVAL;
            return (-1);
        }
...
#ifdef DEBUG
            QMessageBox::warning(0, "WOL", QObject::tr("Invalid ethernet address !"));
#endif
            errno = EINVAL;
            return (-1);
        }
...
int wol::send_wol (QString string_mac)
{
    ...
    if(erreur) {
        QMessageBox::warning(0, "WOL", QObject::tr("Erreur d'initialisation"));
        return(-1);
    }
    ...
    if (in_ether (mac, ethaddr) < 0) {
        QMessageBox::warning(0, "WOL", QObject::tr("Invalid hardware address"));
        return (-1);
    }

    // setup the packet socket
    if ((packet = socket (AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
        QMessageBox::warning(0, "WOL", QObject::tr("Socket failed"));
        return (-2);
    }
    ...
    if (setsockopt (packet, SOL_SOCKET, SO_BROADCAST, (char *)&optval,
                    sizeof (optval)) < 0) {
        QMessageBox::warning(0, "WOL", QObject::tr("Set socket failed"));
        closesocket (packet);
        return (-3);
    }
    ...
    if (sendto (packet, (char *)buf, 102, 0, (struct sockaddr *)&sap, sizeof (sap)) < 0) {
        QMessageBox::warning(0, "WOL", "Sendto failed");
        closesocket (packet);
        return (-4);
    }
    closesocket (packet);
    ...
    WSACleanup();
#endif
    QMessageBox::warning(0, "WOL", QObject::tr("Magic packet sent to ") + string_mac + " !");
    return (0);
}
